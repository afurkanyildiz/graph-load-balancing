...
class Rewrite {
 ...
 public:
  ...
  void writeMain(std::ostream &stream, int segment);
 // void writeFunc(int level, int runnablesStart, int numOfRunnables);
  ...
  int writePart(int levelNum, int rowStartIndex, int rowEndIndex, int levelPart, vector<double>& b);
  ...
  int balanceLevel(int toBeBalanced, int& workloadPerThread);
    ...
    void rewriteRow2(int row, vector<double>& b, set<int>& rewritten, map<int,double>& multipliers, double rewritingMultiplicant, char sign);
