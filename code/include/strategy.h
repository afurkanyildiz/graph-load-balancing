...
/*         cout << "level: " << maxLevel << " row: " << *nextRow << " preds original: " << dag[*nextRow].first.size() << " now: " << parents.size() << " cost: " << costRow << "\n";
           cout << "levelCost[" << maxLevel << "]: " << levelCost[maxLevel] << "\n";*/
         if((levelCost[maxLevel] + costRow) <= avgCostPerLevel) {
           levelCost[maxLevel] += costRow;
//           cout << "levelCost[" << maxLevel << "]: " << levelCost[maxLevel] << " costRow: " << costRow << " avgCostPerLevel: " << avgCostPerLevel << "\n";
           ...
           if(nextRow != levelRows.begin()) {
             //cout << "source level: " << level << " has " << nextRow-levelRows.begin() << " rewritten " << levelRows.size()-(nextRow-levelRows.begin())  << " more in the level\n";
             //cout << "current cost of " << targetLevel->first << " : " << levelCost[targetLevel->first] << "\n";
             float cost = 0;
